...
/*Provider*/
final couponsProvider = StateNotifierProvider<CouponRepository>((ref) => CouponRepository());
final applyCouponProvider = StateNotifierProvider<ApplyCouponRepository>((ref) => ApplyCouponRepository());
...
class CouponRepository extends StateNotifier<CouponState> {
  ...
  Future coupons() async {
    ...
    var responseBody;
    try {
      responseBody = await handleResponse(await getRequest(API.coupons, bearerToken: true));
  ...
  Future applyCoupon(cartId, coupon) async {
    ...
    try {
      responseBody = await handleResponse(await postRequest(
        API.applyCoupon(cartId, coupon),{}, bearerToken: true
      ));
      if (responseBody is int) if (responseBody > 206) throw NetworkException();
