...
class CategoryRepository implements ICategoryRepository {
  ...
  @override
  Future<List<CategorySubgroup>> fetchCategorySubgroupList(String categoryID) async {
    var responseBody = await handleResponse(await getRequest(API.categorySubgroup(categoryID)));
    if (responseBody.runtimeType == int) {
      if (responseBody > 206) {
        throw NetworkException();
      }
    ...
    CategorySubGroupModel categorySubGroupModel = CategorySubGroupModel.fromJson(responseBody);
    return categorySubGroupModel.data;
  }

  @override
  Future<List<SubgroupCategory>> fetchSubgroupCategoryList(String subgroupID) async {
    var responseBody = await handleResponse(await getRequest(API.subgroupCategory(subgroupID)));
    if (responseBody.runtimeType == int) {
      if (responseBody > 206) {
        throw NetworkException();
      }
    }
    SubgroupCategoryModel subgroupCategoryModel = SubgroupCategoryModel.fromJson(responseBody);
...
class CategoryItemRepository implements ICategoryItemRepository {
  ...
  Future<List<CategoryItem>> fetchCategoryItemList(String slug) async {
    var responseBody = await handleResponse(await getRequest(API.categoryItem(slug)));
    if (responseBody.runtimeType == int) {
      if (responseBody > 206) {
        throw NetworkException();
      }
    }
    CategoryItemModel categoryItemModel = CategoryItemModel.fromJson(responseBody);
