...
Future<Response> getRequest(String endPoint,
  ...
  if (await isNetworkAvailable()) {
    ...
    if (bearerToken) {
      response = await get('${API.base}$endPoint', headers: headers);
    } else if (noBaseUrl) {
...
postRequest(String endPoint, Map requestBody,
  ...
  if (await isNetworkAvailable()) {
    ...
    try {
      if (!noBaseUrl) {
        response = await post('${API.base}' + '$endPoint',
            body: requestBody, headers: headers);
      } else {
        response = await post('$endPoint', body: requestBody, headers: headers);
      }
    } catch (e) {
      print(e.toString());
    }
...
putRequest(String endPoint, Map request, {bool bearerToken = true}) async {
  if (await isNetworkAvailable()) {
    ...
    print("Headers: $headers");
    try {
      response = await put('${API.base}' + '$endPoint',
...
deleteRequest(String endPoint, {bool bearerToken = true}) async {
  if (await isNetworkAvailable()) {
    ...
    print(headers);
    Response response =
        await delete('${API.base}' + '$endPoint', headers: headers);
    print('Response: ${response.statusCode} ${response.body}');
    return response;
  } else {
    throw noInternetMsg;
