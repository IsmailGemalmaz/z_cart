...
class _ProductDetailsScreenState extends State<ProductDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return ProviderListener<ProductState>(
        provider: productNotifierProvider.state,
        onChange: (context, state) {
          if (state is ProductLoadedState) {
            context.read(cartNotifierProvider).getCartList();
            _quantity = state.productModel.data.minOrderQuantity;
          }
        },
        child: Consumer(
          builder: (context, watch, _) {
            final productDetailsState = watch(productNotifierProvider.state);

            if (productDetailsState is ProductLoadedState) {
              _totalPrice =
                  double.parse(productDetailsState.productModel.data.rawPrice) *
                      _quantity;
            }
            ...
            return WillPopScope(
              ...
              onWillPop: () {
                ...
                context.read(productSlugListProvider).removeProductSlug();
                if (context.read(productSlugListProvider.state).isNotEmpty)
                  context.read(productNotifierProvider).getProductDetails(
                      context.read(productSlugListProvider.state).last);
                                  ...
                                  children: [
                                    Column(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                          "Total - \$${_totalPrice.toDoubleStringAsPrecised(length: 2)}",
